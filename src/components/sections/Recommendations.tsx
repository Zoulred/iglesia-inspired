    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-6">
        {recommendations.map((rec) => (
          <div key={rec.id} className="bg-gray-50/50 dark:bg-gray-800/30 p-4 rounded-2xl border border-gray-50 dark:border-gray-800">
            <p className="text-gray-600 dark:text-gray-300 italic mb-4 text-sm">"{rec.content}"</p>
            <div className="flex items-center gap-3">
              {rec.avatar && (
                <img src={rec.avatar} alt={rec.name} className="w-10 h-10 rounded-full object-cover" />
              )}
              <div>
                <h4 className="text-sm font-bold text-gray-900 dark:text-white">{rec.name}</h4>
          ...
          <div className="space-y-3">
            {profileData.socials.map((social) => (
              <a
                ...
                className="flex items-center justify-between gap-4 p-3 bg-stone-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-md hover:shadow-sm dark:hover:shadow-gray-900/50 transition"
              >
                <div className="flex items-center gap-3">
                  <span className="w-9 h-9 flex items-center justify-center rounded-md bg-gray-50 dark:bg-gray-700 border border-gray-100 dark:border-gray-600 text-gray-700 dark:text-gray-200">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-white">{social.platform}</span>
                </div>
